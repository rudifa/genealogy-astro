...
import { loadSampleTree, appDataEnsureExists } from '../public/utility/AppDataLoader.js';
...
describe('loadSampleTree', () => {
  it('fetches and returns parsed JSON', async () => {
    const data = await loadSampleTree();
    expect(global.fetch).toHaveBeenCalledWith('/utility/data/sample_tree.json');
...
describe('appDataEnsureExists', () => {
  it('creates window.appData and initializes it (real AppData)', async () => {
    ...
    expect(appData.state.genealogyData.persons || appData.state.genealogyData._persons).toBeDefined();
